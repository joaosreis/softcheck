...
module MakeCfg(F : sig
  ...
  let outflow {flow=g} l = G.succ g l
  ...
  let add {blocks=b; flow=g; functions=p} id (l, v) = Hashtbl.replace b l v; Hashtbl.replace p l id; G.add_vertex g l
  let connect {flow=g} ?(label = E.default) l l' = G.add_edge_e g (G.E.create l label l')
  let get_blocks {blocks=b} = b
  let get_func_id {functions=p} l = Hashtbl.find p l

  let dot_output {blocks=b; flow=g; functions=p} f =
    let module Helper =
    struct
