...
let contains_ident x =
  let rec contains_ident_apat acc = function
      VectP (CElem e) -> contains_ident_rec acc e
    | VectP (CRange r) -> contains_ident_rec (contains_ident_rec acc r.crange_start_val) r.crange_end_val
    | MatP m -> contains_ident_apat (contains_ident_apat acc (VectP m.matp_row)) (VectP m.matp_col)
  and contains_ident_rec acc = function
      Var _ as i -> acc || i = x
    | FunCall f -> List.fold_left contains_ident_rec acc f.funcall_args
    | Lit _ | StructProj _ -> false
    | UnaryOp unop -> contains_ident_rec acc unop.unaryop_expr
    | BinaryOp binop -> contains_ident_rec (contains_ident_rec acc binop.binaryop_l_expr) binop.binaryop_r_expr
    | Access a -> contains_ident_apat acc a.access_pattern
    | Cast c -> contains_ident_rec acc c.cast_expr
  in contains_ident_rec false

let rec get_non_trivial_subexpressions acc = let open Set.Infix in function
    Var _
  | Lit _
  | FunCall _
  | StructProj _
